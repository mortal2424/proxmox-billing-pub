...
try {
    ...
    switch ($action) {
        case 'start':
            $result = $proxmoxApi->startVM($vmid);
            break;
        case 'stop':
            $result = $proxmoxApi->stopVM($vmid);
            break;
        case 'reboot':
            $result = $proxmoxApi->rebootVM($vmid);
            break;
        case 'get_ip':
            $result = $proxmoxApi->getVmIp($vmid);
            break;
        default:
            throw new Exception('Invalid action');
    }

    // Обновляем статус VM в базе данных
    if (in_array($action, ['start', 'stop', 'reboot'])) {
        $newStatus = $action === 'start' ? 'running' : ($action === 'stop' ? 'stopped' : 'running');
        $stmt = $pdo->prepare("UPDATE vms SET status = ? WHERE vm_id = ?");
