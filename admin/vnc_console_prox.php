...
ini_set('error_log', __DIR__ . '/../logs/vnc_errors_prox.log');
...
// Класс для работы с VNC (можно использовать тот же что и в vnc_console_admin.php)
class VncHelperProx {
    public static function getVncTicket($host, $port, $nodeName, $vmId, $vmType, $pveTicket, $csrfToken) {
        $logMessage = "Starting getVncTicket (Prox):\n";
...
try {
    ...
    error_log("========== Starting PROXMOX DIRECT VNC Request ==========");
    ...
    $nodeData = $nodeStmt->fetch();

    if (!$nodeData) {
        throw new Exception("Нода с ID $nodeId не найдена");
    }

    error_log("Node data from DB: " . print_r($nodeData, true));

    // Проверяем наличие обязательных полей
    if (empty($nodeData['hostname']) || empty($nodeData['node_name'])) {
    ...
    // Создаем объект ProxmoxAPI для проверки ВМ и определения её типа
    $proxmox = new ProxmoxAPI(
        $nodeData['hostname'],
        $nodeData['username'],
        $nodeData['password'],
        $nodeData['ssh_port'] ?? 22,
        $nodeData['node_name'],
        $nodeId,
        $pdo
    );

    // Получаем информацию о ВМ для определения её типа
    error_log("Getting VM info for VMID: $vmId");
    $vmInfo = $proxmox->getVmInfo($vmId);
    
    if (!$vmInfo) {
        throw new Exception("ВМ с ID $vmId не найдена на ноде {$nodeData['node_name']}");
    }

    // Определяем тип ВМ (qemu или lxc)
    $vmType = $vmInfo['type'] ?? 'qemu';
    error_log("VM type determined: $vmType");

    // Определяем хост для подключения
    $urlHost = $nodeData['hostname'];
    $urlPort = $nodeData['api_port'] ?? 8006;

    // Если нода в кластере, используем мастер-ноду
    ...
    error_log("Attempting to get VNC ticket for Proxmox VM:");
    error_log("Host: $urlHost, Port: $urlPort, Node: {$nodeData['node_name']}, VMID: $vmId, Type: $vmType");

    $vncData = VncHelperProx::getVncTicket(
        $urlHost,
        $urlPort,
        $nodeData['node_name'],
        $vmId,
    ...
    error_log("Exception in PROXMOX VNC: " . $e->getMessage() . "\nTrace: " . $e->getTraceAsString());
    http_response_code(500);
    echo json_encode([
        'success' => false,
        'error' => $e->getMessage(),
        'details' => [
            'is_proxmox_direct' => true,
            'vm_id' => $vmId ?? null,
            'node_id' => $nodeId ?? null
        ]
    ]);
